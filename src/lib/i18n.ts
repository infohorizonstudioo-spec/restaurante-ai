...
const TX: Record<string, Record<string, string>> = {
  ...
  'Reserva': {en:'Reservation',fr:'Réservation',pt:'Reserva',ca:'Reserva'},
  'Cita': {en:'Appointment',fr:'Rendez-vous',pt:'Consulta',ca:'Cita'},
  'Pedido': {en:'Order',fr:'Commande',pt:'Pedido',ca:'Comanda'},
  'Cancelación': {en:'Cancellation',fr:'Annulation',pt:'Cancelamento',ca:'Cancel·lació'},
  'Consulta': {en:'Inquiry',fr:'Question',pt:'Consulta',ca:'Consulta'},
  'Visita': {en:'Visit',fr:'Visite',pt:'Visita',ca:'Visita'},
  // Panel — CALL_STATE_MAP labels
  'Escuchando': {en:'Listening',fr:'Écoute',pt:'A ouvir',ca:'Escoltant'},
  'Hablando': {en:'Speaking',fr:'Parle',pt:'A falar',ca:'Parlant'},
  'Procesando': {en:'Processing',fr:'Traitement',pt:'A processar',ca:'Processant'},
  'Finalizando': {en:'Finishing',fr:'Finalisation',pt:'A finalizar',ca:'Finalitzant'},
  // Panel — realtime events
  'Completada': {en:'Completed',fr:'Terminée',pt:'Concluída',ca:'Completada'},
  'detectada': {en:'detected',fr:'détectée',pt:'detetada',ca:'detectada'},
  'finalizada': {en:'completed',fr:'terminée',pt:'finalizada',ca:'finalitzada'},
  'Resumen generado': {en:'Summary generated',fr:'Résumé généré',pt:'Resumo gerado',ca:'Resum generat'},
  'Requiere atención inmediata': {en:'Requires immediate attention',fr:'Nécessite une attention immédiate',pt:'Requer atenção imediata',ca:'Requereix atenció immediata'},
  'cancelada': {en:'cancelled',fr:'annulée',pt:'cancelada',ca:'cancel·lada'},
  'no presentado': {en:'no show',fr:'absent',pt:'não compareceu',ca:'no presentat'},
  'Llamada entrante': {en:'Incoming call',fr:'Appel entrant',pt:'Chamada recebida',ca:'Trucada entrant'},
  'Llamada finalizada': {en:'Call ended',fr:'Appel terminé',pt:'Chamada finalizada',ca:'Trucada finalitzada'},
  'Pedido confirmado': {en:'Order confirmed',fr:'Commande confirmée',pt:'Pedido confirmado',ca:'Comanda confirmada'},
  'Cita confirmada': {en:'Appointment confirmed',fr:'Rendez-vous confirmé',pt:'Consulta confirmada',ca:'Cita confirmada'},
  'Reserva cancelada': {en:'Reservation cancelled',fr:'Réservation annulée',pt:'Reserva cancelada',ca:'Reserva cancel·lada'},
  'Nuevo pedido —': {en:'New order —',fr:'Nouvelle commande —',pt:'Novo pedido —',ca:'Nova comanda —'},
  'tomando pedido…': {en:'taking order…',fr:'prise de commande…',pt:'a anotar pedido…',ca:'prenent comanda…'},
  'Tomando pedido...': {en:'Taking order...',fr:'Prise de commande...',pt:'A anotar pedido...',ca:'Prenent comanda...'},
  'Recogida': {en:'Pickup',fr:'Retrait',pt:'Recolha',ca:'Recollida'},
  'recoger': {en:'pickup',fr:'à emporter',pt:'levantar',ca:'recollir'},
  'pedidos en curso': {en:'orders in progress',fr:'commandes en cours',pt:'pedidos em curso',ca:'comandes en curs'},
  'pedido en curso': {en:'order in progress',fr:'commande en cours',pt:'pedido em curso',ca:'comanda en curs'},
  'consultas en curso': {en:'consultations in progress',fr:'consultations en cours',pt:'consultas em curso',ca:'consultes en curs'},
  'consulta en curso': {en:'consultation in progress',fr:'consultation en cours',pt:'consulta em curso',ca:'consulta en curs'},
  'Ver citas': {en:'View appointments',fr:'Voir rendez-vous',pt:'Ver consultas',ca:'Veure cites'},
  'Ver pedidos': {en:'View orders',fr:'Voir commandes',pt:'Ver pedidos',ca:'Veure comandes'},
  'URGENTE': {en:'URGENT',fr:'URGENT',pt:'URGENTE',ca:'URGENT'},
  'Primera visita': {en:'First visit',fr:'Première visite',pt:'Primeira visita',ca:'Primera visita'},
  'En curso': {en:'In progress',fr:'En cours',pt:'Em curso',ca:'En curs'},
  'hoy': {en:'today',fr:"aujourd'hui",pt:'hoje',ca:'avui'},
  'Las': {en:'',fr:'Les',pt:'As',ca:'Les'},
  'Sin': {en:'No',fr:'Pas de',pt:'Sem',ca:'Sense'},
  'se mostrarán automáticamente cuando entren.': {en:'will appear automatically when they come in.',fr:'apparaîtront automatiquement.',pt:'aparecerão automaticamente quando chegarem.',ca:'es mostraran automàticament quan entrin.'},
  'Uso del trial gratuito': {en:'Free trial usage',fr:"Utilisation de l'essai gratuit",pt:'Uso do teste gratuito',ca:'Ús del trial gratuït'},
  'Cada llamada recibida cuenta como una del plan.': {en:'Each received call counts as one from the plan.',fr:'Chaque appel reçu compte comme un du plan.',pt:'Cada chamada recebida conta como uma do plano.',ca:'Cada trucada rebuda compta com una del pla.'},
  'Trial agotado': {en:'Trial exhausted',fr:'Essai épuisé',pt:'Teste esgotado',ca:'Trial exhaurit'},
  'llamadas restantes': {en:'calls remaining',fr:'appels restants',pt:'chamadas restantes',ca:'trucades restants'},
  'llamada restante': {en:'call remaining',fr:'appel restant',pt:'chamada restante',ca:'trucada restant'},
  'Activa un plan para seguir recibiendo llamadas sin límites': {en:'Activate a plan to keep receiving unlimited calls',fr:'Activez un plan pour continuer à recevoir des appels',pt:'Ative um plano para continuar a receber chamadas sem limites',ca:'Activa un pla per continuar rebent trucades sense límits'},
  'Ver planes': {en:'View plans',fr:'Voir les plans',pt:'Ver planos',ca:'Veure plans'},
  'llamadas en curso': {en:'calls in progress',fr:'appels en cours',pt:'chamadas em curso',ca:'trucades en curs'},
  'llamada en curso': {en:'call in progress',fr:'appel en cours',pt:'chamada em curso',ca:'trucada en curs'},
  'Tiempo real': {en:'Real time',fr:'Temps réel',pt:'Tempo real',ca:'Temps real'},
  'Salir demo': {en:'Exit demo',fr:'Quitter démo',pt:'Sair demo',ca:'Sortir demo'},
  'Modo demo': {en:'Demo mode',fr:'Mode démo',pt:'Modo demo',ca:'Mode demo'},
  'Ver demo': {en:'View demo',fr:'Voir démo',pt:'Ver demo',ca:'Veure demo'},
  'El sistema está listo': {en:'System is ready',fr:'Le système est prêt',pt:'O sistema está pronto',ca:'El sistema està llest'},
  'Tu recepcionista está activa y preparada. Los eventos aparecerán aquí en tiempo real.': {en:'Your receptionist is active and ready. Events will appear here in real time.',fr:'Votre réceptionniste est actif et prêt. Les événements apparaîtront ici en temps réel.',pt:'O rececionista está ativo e preparado. Os eventos aparecerão aqui em tempo real.',ca:'La recepcionista està activa i preparada. Els esdeveniments apareixeran aquí en temps real.'},
  'activa': {en:'active',fr:'actif',pt:'ativo',ca:'activa'},
  'Sin número asignado': {en:'No number assigned',fr:'Pas de numéro assigné',pt:'Sem número atribuído',ca:'Sense número assignat'},
  'Configurar': {en:'Configure',fr:'Configurer',pt:'Configurar',ca:'Configurar'},
  'Nuevo pedido de': {en:'New order from',fr:'Nouvelle commande de',pt:'Novo pedido de',ca:'Nova comanda de'},
  'Para': {en:'For',fr:'Pour',pt:'Para',ca:'Per a'},
  'Toca para ver en pedidos': {en:'Tap to view in orders',fr:'Appuyez pour voir les commandes',pt:'Toque para ver nos pedidos',ca:'Toca per veure a comandes'},
  'Esperando llamadas. Cuando entren, aparecerán aquí en tiempo real con su resumen.': {en:'Waiting for calls. When they come in, they will appear here in real time with their summary.',fr:'En attente d\'appels. Ils apparaîtront ici en temps réel avec leur résumé.',pt:'À espera de chamadas. Quando chegarem, aparecerão aqui em tempo real com o resumo.',ca:'Esperant trucades. Quan entrin, apareixeran aquí en temps real amb el resum.'},
  'Configura tu número de teléfono para empezar a recibir llamadas.': {en:'Set up your phone number to start receiving calls.',fr:'Configurez votre numéro de téléphone pour commencer à recevoir des appels.',pt:'Configure o seu número de telefone para começar a receber chamadas.',ca:'Configura el teu número de telèfon per començar a rebre trucades.'},
  'Configurar número': {en:'Set up number',fr:'Configurer numéro',pt:'Configurar número',ca:'Configurar número'},
  'Responde 24/7': {en:'Answers 24/7',fr:'Répond 24/7',pt:'Atende 24/7',ca:'Respon 24/7'},
  'Detecta reservas': {en:'Detects reservations',fr:'Détecte les réservations',pt:'Deteta reservas',ca:'Detecta reserves'},
  'Toma pedidos': {en:'Takes orders',fr:'Prend les commandes',pt:'Anota pedidos',ca:'Pren comandes'},
  // Demo events
  'Llamada entrante —': {en:'Incoming call —',fr:'Appel entrant —',pt:'Chamada recebida —',ca:'Trucada entrant —'},
  'Reserva detectada': {en:'Reservation detected',fr:'Réservation détectée',pt:'Reserva detetada',ca:'Reserva detectada'},
  'Nueva reserva — María López': {en:'New reservation — María López',fr:'Nouvelle réservation — María López',pt:'Nova reserva — María López',ca:'Nova reserva — María López'},
  '4 personas · 21:00': {en:'4 people · 21:00',fr:'4 personnes · 21:00',pt:'4 pessoas · 21:00',ca:'4 persones · 21:00'},
  'Llamada finalizada — Carlos Ruiz': {en:'Call ended — Carlos Ruiz',fr:'Appel terminé — Carlos Ruiz',pt:'Chamada finalizada — Carlos Ruiz',ca:'Trucada finalitzada — Carlos Ruiz'},
  'Reserva confirmada para mañana': {en:'Reservation confirmed for tomorrow',fr:'Réservation confirmée pour demain',pt:'Reserva confirmada para amanhã',ca:'Reserva confirmada per demà'},
  'Consulta sobre el menú': {en:'Menu inquiry',fr:'Question sur le menu',pt:'Consulta sobre o menu',ca:'Consulta sobre el menú'},
  'Cliente preguntó por opciones vegetarianas': {en:'Client asked about vegetarian options',fr:'Client a demandé les options végétariennes',pt:'Cliente perguntou por opções vegetarianas',ca:'Client va preguntar per opcions vegetarianes'},
  'Consulta detectada': {en:'Inquiry detected',fr:'Question détectée',pt:'Consulta detetada',ca:'Consulta detectada'},
  'pendiente': {en:'pending',fr:'en attente',pt:'pendente',ca:'pendent'},
  'Urgencia': {en:'Urgency',fr:'Urgence',pt:'Urgência',ca:'Urgència'},
  'consulta': {en:'consultation',fr:'consultation',pt:'consulta',ca:'consulta'},
  'Cliente': {en:'Client',fr:'Client',pt:'Cliente',ca:'Client'},
  'Paciente': {en:'Patient',fr:'Patient',pt:'Paciente',ca:'Pacient'},
}
