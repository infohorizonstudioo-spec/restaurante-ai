...
const NAV = [
  ...
  { href:'/pedidos',     icon:'shopping',  label:'Pedidos',     pro:true },
  { href:'/estadisticas',icon:'bar',       label:'Estadisticas',pro:true },
  { href:'/facturacion', icon:'credit',    label:'Facturacion' },
...
const ICONS:Record<string,string> = {
  ...
  credit:   'M21 4H3a2 2 0 00-2 2v12a2 2 0 002 2h18a2 2 0 002-2V6a2 2 0 00-2-2zM1 10h22',
...
export function Sidebar(){
  ...
  useEffect(()=>{
    (async()=>{
      ...
      // Usar RPC get_billing_summary para datos exactos del backend
      const {data:b} = await supabase.rpc('get_billing_summary',{p_tenant_id:p.tenant_id})
      setBilling(b)
      const {data:t} = await supabase.from('tenants').select('agent_phone,plan').eq('id',p.tenant_id).single()
      setAgentActive(!!t?.agent_phone)
    })()
  },[])
  ...
  const isTrial = billing?.is_trial ?? true
  const usedCalls = billing?.used_calls || 0
  const includedCalls = billing?.included_calls || 10
  const usedPct = includedCalls > 0 ? Math.min(100, Math.round((usedCalls/includedCalls)*100)) : 0
  const extraCalls = billing?.extra_calls || 0
  const planColor = PLAN_COL[plan] || '#d97706'
  ...
  return(
    <nav style={{width:collapsed?56:220,minWidth:collapsed?56:220,background:'#0f172a',display:'flex',flexDirection:'column',height:'100vh',position:'sticky',top:0,transition:'width 0.2s',overflow:'hidden',flexShrink:0}}>
      ...
      <div style={{padding:collapsed?'14px 10px':'14px 16px',borderBottom:'1px solid #1e293b',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        {!collapsed&&<span style={{fontSize:16,fontWeight:800,color:'white',letterSpacing:'-0.025em'}}>Reservo.AI</span>}
        <button onClick={()=>setCollapsed(c=>!c)} style={{background:'none',border:'none',cursor:'pointer',color:'#64748b',padding:4,borderRadius:6,display:'flex',alignItems:'center',justifyContent:'center',marginLeft:collapsed?'auto':0}}>
      ...
      {!collapsed&&(
        <div style={{padding:'8px 14px',borderBottom:'1px solid #1e293b'}}>
          <div style={{display:'flex',alignItems:'center',gap:6}}>
            <div style={{width:7,height:7,borderRadius:'50%',background:agentActive?'#4ade80':'#ef4444',flexShrink:0}}/>
      ...
      <div style={{flex:1,overflow:'auto',padding:'8px 0'}}>
        {NAV.map(item=>{
          ...
          const locked = isPro && isTrial
          return(
            <Link key={item.href} href={locked?'/precios':item.href}
              style={{display:'flex',alignItems:'center',gap:10,padding:collapsed?'10px 18px':'9px 14px',margin:'1px 6px',borderRadius:8,textDecoration:'none',background:active?'#1e293b':'transparent',color:active?'white':'#94a3b8',transition:'all 0.1s',position:'relative'}}>
              <span style={{color:active?'white':'#64748b',flexShrink:0}}><Icon name={item.icon}/></span>
              {!collapsed&&(
                <>
                  ...
                  {isPro&&<span style={{fontSize:9,fontWeight:700,padding:'1px 5px',borderRadius:5,background:'#7c3aed22',color:'#a78bfa',flexShrink:0}}>PRO</span>}
                </>
              )}
            </Link>
          )
      ...
      {!collapsed&&billing&&(
        <div style={{padding:'10px 14px',borderTop:'1px solid #1e293b'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:4}}>
            ...
            <span style={{fontSize:10,color:'#64748b'}}>{usedCalls}/{includedCalls}</span>
          </div>
          <div style={{height:4,background:'#1e293b',borderRadius:2,overflow:'hidden',marginBottom:4}}>
            <div style={{height:'100%',width:usedPct+'%',background:usedPct>=100?'#ef4444':usedPct>=80?'#f59e0b':planColor,borderRadius:2,transition:'width 0.5s'}}/>
          </div>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            <span style={{fontSize:10,color:'#475569'}}>LLAMADAS</span>
            {extraCalls>0&&<span style={{fontSize:10,color:'#ef4444',fontWeight:600}}>+{extraCalls} extra</span>}
      ...
      <div style={{padding:'8px 6px',borderTop:'1px solid #1e293b'}}>
        <button onClick={handleLogout} style={{width:'100%',display:'flex',alignItems:'center',gap:10,padding:collapsed?'8px 14px':'8px 10px',borderRadius:8,background:'none',border:'none',cursor:'pointer',color:'#64748b'}}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
