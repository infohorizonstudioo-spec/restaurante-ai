...
export default function Sidebar() {
  ...
  // billing — leído del tenant directamente (sin RPC extra en el sidebar)
  const plan: string    = tenant?.plan || 'trial'
  const isTrial: boolean = plan === 'trial' || plan === 'free'
  const used: number    = isTrial ? (tenant?.free_calls_used || 0) : (tenant?.plan_calls_used || 0)
  const included: number= isTrial ? (tenant?.free_calls_limit || 10) : (tenant?.plan_calls_included || 50)
  ...
  return (
    <nav style={{ width: collapsed ? 56 : 220, minWidth: collapsed ? 56 : 220, background: '#0f172a', display: 'flex', flexDirection: 'column', height: '100vh', position: 'sticky', top: 0, transition: 'width 0.2s', overflow: 'hidden', flexShrink: 0 }}>
      ...
      <div style={{ padding: collapsed ? '14px 10px' : '14px 16px', borderBottom: '1px solid #1e293b', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        {!collapsed && <span style={{ fontSize: 16, fontWeight: 800, color: 'white', letterSpacing:'-0.02em' }}>Reservo.AI</span>}
        ...
        <div style={{ padding: '8px 14px', borderBottom: '1px solid #1e293b' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ fontSize: 11, color: agentActive ? '#4ade80' : '#ef4444', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {agentActive ? agentName + ' activa' : 'Sin número configurado'}
            </span>
          </div>
        </div>
      )}

      {/* Navegación — generada desde la plantilla */}
        ...
        {modules.map(item => {
          ...
          return (
            <Link key={item.href} href={locked ? '/precios' : item.href}
              ...
              <span style={{ color: active ? 'white' : '#64748b', flexShrink: 0 }}>
                <SvgIcon d={ICONS[item.icon] || ICONS.grid} />
              </span>
              {!collapsed && (
                <>
      ...
      {!collapsed && tenant && (
        <div style={{ padding: '10px 14px', borderTop: '1px solid #1e293b' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: 10, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Llamadas</span>
