...
export function Modal({open,onClose,title,children,footer,size='md'}:{open:boolean;onClose:()=>void;title:string;children:ReactNode;footer?:ReactNode;size?:string}){
  ...
  return(<div style={{position:'fixed',inset:0,zIndex:200,background:'rgba(0,0,0,0.75)',backdropFilter:'blur(4px)',display:'flex',alignItems:'center',justifyContent:'center',padding:16}} onClick={e=>e.target===e.currentTarget&&onClose()}><div style={{background:RZ.surface,border:`1px solid ${RZ.borderMd}`,borderRadius:16,boxShadow:'0 25px 60px rgba(0,0,0,0.6)',width:'100%',maxWidth:mw[size]||560,maxHeight:'90vh',overflowY:'auto'}}><div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'16px 24px',borderBottom:`1px solid ${RZ.border}`}}><p style={{fontSize:15,fontWeight:700,color:RZ.text}}>{title}</p><button onClick={onClose} style={{width:28,height:28,display:'flex',alignItems:'center',justifyContent:'center',borderRadius:6,border:'none',background:'transparent',cursor:'pointer',fontSize:18,color:RZ.text3}} aria-label="Cerrar">&times;</button></div><div style={{padding:'20px 24px'}}>{children}</div>{footer&&<div style={{display:'flex',gap:10,padding:'14px 24px',borderTop:`1px solid ${RZ.border}`,background:RZ.surface2,borderRadius:'0 0 16px 16px'}}>{footer}</div>}</div></div>)
}
