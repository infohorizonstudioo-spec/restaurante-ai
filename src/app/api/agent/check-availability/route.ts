...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!tenant_id || !date) {
      return NextResponse.json({ error: "tenant_id and date required" }, { status: 400 })
    }

    logger.info('agent:check-availability', { tenant_id, date, time, party_size })

    const result = await checkAvailabilityTool({ tenant_id, date, time, party_size, zone })
