...
export default function PanelPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (!p?.tenant_id) { router.push('/onboarding'); return }
    if (p.role === 'superadmin') { router.push('/admin'); return }
    ...
    const [{ data: t }, { data: c }, { data: r }, { data: cl }] = await Promise.all([
      ...
      supabase.from('reservations').select('*').eq('tenant_id', tid).eq('reservation_date', today).order('reservation_time'),
  ...
  return (
    <div style={{ background:'#f8fafc', minHeight:'100vh' }}>
      ...
      <div style={{ background:'white', borderBottom:'1px solid #e2e8f0', padding:'16px 28px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <div>
          <h1 style={{ fontSize:18, fontWeight:700, color:'#0f172a', letterSpacing:'-0.015em' }}>
            {greeting}, <span style={{ color:'#1d4ed8' }}>{tenant.name}</span>
          </h1>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          {!agentActive && (
            <Link href="/configuracion" style={{ padding:'6px 14px', fontSize:12, fontWeight:600, color:'white', background:'linear-gradient(135deg,#1e40af,#3b82f6)', borderRadius:8, textDecoration:'none' }}>
              Configurar →
            </Link>
          )}
        </div>
      ...
      <div style={{ maxWidth:1100, margin:'0 auto', padding:'24px 28px' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              ...
              <div>
                <p style={{ fontWeight:600, fontSize:14, color:'#92400e' }}>
                  {callsLeft === 0 ? 'Has agotado tus llamadas gratuitas' : `Quedan solo ${callsLeft} llamada${callsLeft!==1?'s':''} gratuita${callsLeft!==1?'s':''}`}
                </p>
                <p style={{ fontSize:12, color:'#b45309', marginTop:1 }}>Activa un plan para seguir recibiendo llamadas</p>
              </div>
            </div>
            <Link href="/precios" style={{ padding:'8px 18px', fontSize:13, fontWeight:600, color:'white', background:'linear-gradient(135deg,#d97706,#f59e0b)', borderRadius:9, textDecoration:'none', whiteSpace:'nowrap' }}>
              Ver planes →
            </Link>
          </div>
        )}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:20 }}>
          <StatCard label="Llamadas hoy" value={calls.filter(c=>{const d=c.created_at?.slice(0,10);return d===new Date().toISOString().split('T')[0]}).length} color='#1d4ed8' href='/llamadas'/>
          ...
          <StatCard label="Clientes registrados" value={clientes.length} color='#7c3aed' href='/clientes'/>
          <StatCard label="Llamadas del plan" value={isTrial ? callsLeft+' restantes' : callsUsed+'/'+callsLimit} color={callsLeft<=3?'#dc2626':planColor} sub={planLabel} href='/precios'/>
        </div>

        {/* Call usage bar (solo si es trial) */}
        {isTrial && (
          <div style={{ background:'white', border:'1px solid #e2e8f0', borderRadius:12, padding:'16px 20px', marginBottom:20 }}>
            ...
            <div style={{ height:8, background:'#f1f5f9', borderRadius:4, overflow:'hidden', marginBottom:8 }}>
              <div style={{ height:'100%', width:callsPct+'%', background: callsLeft<=3 ? '#ef4444' : callsPct>70 ? '#f59e0b' : '#1d4ed8', borderRadius:4, transition:'width 0.5s' }}/>
            </div>
              ...
              : calls.map((call, i) => {
                  ...
                  return (
                    <div key={call.id} style={{ display:'flex', alignItems:'flex-start', gap:12, padding:'12px 20px', borderTop: i>0 ? '1px solid #f8fafc' : 'none' }}>
                      <div style={{ width:34, height:34, borderRadius:'50%', background: isCompleted ? '#f0fdf4' : '#fef2f2', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill={isCompleted?'#059669':'#dc2626'}><path d="M6.6 10.8c1.4 2.8 3.8 5.1 6.6 6.6l2.2-2.2c.3-.3.7-.4 1-.2 1.1.4 2.3.6 3.6.6.6 0 1 .4 1 1V20c0 .6-.4 1-1 1-9.4 0-17-7.6-17-17 0-.6.4-1 1-1h3.5c.6 0 1 .4 1 1 0 1.3.2 2.5.6 3.6.1.3 0 .7-.2 1L6.6 10.8z"/></svg>
                      </div>
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:2 }}>
                          <p style={{ fontSize:13, fontWeight:600, color:'#0f172a' }}>{call.from_number || 'Número oculto'}</p>
                          <span style={{ fontSize:11, padding:'1px 7px', borderRadius:10, background: isCompleted?'#f0fdf4':'#fef2f2', color: isCompleted?'#059669':'#dc2626', fontWeight:600 }}>
                            {isCompleted ? 'Completada' : call.status === 'in-progress' ? 'En curso' : 'Fallida'}
                          </span>
                        </div>
                        {call.summary
                          ? <p style={{ fontSize:12, color:'#64748b', lineHeight:1.5, display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', overflow:'hidden' }}>{call.summary}</p>
                          : <p style={{ fontSize:12, color:'#94a3b8' }}>Sin resumen</p>
                        }
                      </div>
                      <div style={{ flexShrink:0, textAlign:'right' }}>
                        <p style={{ fontSize:11, color:'#94a3b8', whiteSpace:'nowrap' }}>
                          {call.created_at ? new Date(call.created_at).toLocaleTimeString('es-ES',{hour:'2-digit',minute:'2-digit'}) : ''}
                        </p>
                        {call.duration && <p style={{ fontSize:11, fontWeight:600, color:'#374151', marginTop:2 }}>{Math.round(call.duration/60)}m {call.duration%60}s</p>}
                      </div>
                    </div>
                  )
                })
          ...
          <div style={{ background:'white', border:'1px solid #e2e8f0', borderRadius:14, overflow:'hidden' }}>
            ...
            {reservas.length === 0
              ? <div style={{ padding:'40px 16px', textAlign:'center' }}>
                  ...
                  <p style={{ fontSize:13, color:'#94a3b8', lineHeight:1.6 }}>Sin citas hoy</p>
                </div>
              : reservas.slice(0,8).map((r, i) => (
                  <div key={r.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 16px', borderTop: i>0 ? '1px solid #f8fafc' : 'none' }}>
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      ...
                      <p style={{ fontSize:11, color:'#94a3b8' }}>{r.reservation_time?.slice(0,5)} · {r.party_size}p{r.table_name?' · '+r.table_name:''}</p>
                    </div>
                    <span style={{ fontSize:10, padding:'2px 7px', borderRadius:10, background: r.status==='confirmada'?'#f0fdf4':'#f8fafc', color: r.status==='confirmada'?'#059669':'#94a3b8', fontWeight:600, flexShrink:0 }}>
                      {r.status}
                    </span>
                  </div>
                ))
