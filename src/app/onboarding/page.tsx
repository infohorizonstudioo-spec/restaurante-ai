...
export default function OnboardingPage() {
  ...
  const provisionMessages = ['Creando tu recepcionista...','Configurando horarios...','Preparando canales...','¡Listo!']

  async function activate(){
    if(!tid||saving)return; setSaving(true); setProvisionStep(0)
    const interval=setInterval(()=>setProvisionStep(p=>Math.min(p+1,3)),1800)
    try{
    ...
    }finally{clearInterval(interval);setProvisionStep(3);setSaving(false)}
  }
  ...
  case 'welcome':return(<div style={{textAlign:'center',maxWidth:500,margin:'0 auto'}}><style>{`@keyframes rz-bounce{0%,100%{transform:translateY(0) scale(1)}25%{transform:translateY(-8px) scale(1.05)}50%{transform:translateY(0) scale(1)}75%{transform:translateY(-4px) scale(1.02)}}`}</style><div style={{fontSize:64,marginBottom:16,animation:'rz-bounce 2.5s ease-in-out infinite'}}>🤖</div><h1 style={{fontSize:28,fontWeight:800,color:C.text,lineHeight:1.3}}>Tu negocio, siempre atendido.<br/><span style={{color:C.amber}}>Crea tu recepcionista IA</span></h1><p style={{fontSize:15,color:C.sub,marginTop:16,lineHeight:1.6}}>En unos minutos tendrás todo listo para que tu negocio reciba llamadas, mensajes y reservas automáticamente.</p><div style={{marginTop:32,display:'flex',flexDirection:'column',gap:12,textAlign:'left',background:C.card2,borderRadius:14,padding:'20px 24px'}}>{['Solo te pediremos lo importante','Todo se guarda automáticamente','Puedes cambiar cualquier cosa después','En 10 minutos tendrás tu sistema listo'].map((t,i)=>(<div key={i} style={{display:'flex',alignItems:'center',gap:10}}><span style={{color:C.green,fontSize:16}}>✓</span><span style={{fontSize:13,color:C.text}}>{t}</span></div>))}</div></div>)

  case 'type':{const filteredTypes=typeFilter.trim()?BUSINESS_TYPES.filter(bt=>bt.label.toLowerCase().includes(typeFilter.toLowerCase())||bt.desc.toLowerCase().includes(typeFilter.toLowerCase())||bt.id.includes(typeFilter.toLowerCase())):BUSINESS_TYPES;return(<div><h2 style={{fontSize:22,fontWeight:800,color:C.text,marginBottom:6}}>¿Qué tipo de negocio tienes?</h2><p style={{fontSize:13,color:C.sub,marginBottom:16}}>Esto adaptará todo el sistema a tu actividad</p><div style={{position:'relative',marginBottom:20,maxWidth:400}}><input className="rz-inp" value={typeFilter} onChange={e=>setTypeFilter(e.target.value)} placeholder="Buscar tipo de negocio..." style={{paddingLeft:36}}/><span style={{position:'absolute',left:12,top:'50%',transform:'translateY(-50%)',fontSize:14,color:C.muted,pointerEvents:'none'}}>🔍</span></div><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(170px,1fr))',gap:10}}>{filteredTypes.map(bt=>{const sel=d.business_type===bt.id;return(<button key={bt.id} onClick={()=>up('business_type',bt.id)} style={{display:'flex',flexDirection:'column',alignItems:'center',gap:6,padding:'16px 12px',borderRadius:12,cursor:'pointer',fontFamily:'inherit',border:`2px solid ${sel?C.amber:C.border}`,background:sel?C.amberDim:'rgba(255,255,255,0.02)',transition:'all 0.15s'}}><span style={{fontSize:28}}>{bt.icon}</span><span style={{fontSize:13,fontWeight:700,color:sel?C.amber:C.text}}>{bt.label}</span><span style={{fontSize:10,color:C.muted,textAlign:'center'}}>{bt.desc}</span></button>)})}</div>{filteredTypes.length===0&&(<p style={{textAlign:'center',color:C.muted,fontSize:13,marginTop:20}}>No se encontró ningún tipo. Prueba con otro término o selecciona &quot;Otro tipo&quot;.</p>)}</div>)}

  case 'info':return(<div><h2 style={{fontSize:22,fontWeight:800,color:C.text,marginBottom:6}}>Datos de tu negocio</h2><p style={{fontSize:13,color:C.sub,marginBottom:24}}>Lo básico para que tu recepcionista sepa quién eres</p><div style={{display:'flex',flexDirection:'column',gap:16,maxWidth:500}}><Inp label="Nombre del negocio" value={d.business_name} onChange={v=>up('business_name',v)} placeholder="Ej: Restaurante La Tahona"/><div><Inp label="Teléfono de tu negocio" value={d.phone} onChange={v=>up('phone',v)} placeholder="+34 912 345 678"/><div style={{marginTop:8,padding:'10px 14px',background:'rgba(45,212,191,0.06)',border:'1px solid rgba(45,212,191,0.15)',borderRadius:10}}><p style={{fontSize:11,color:C.teal,lineHeight:1.5}}>📞 Este es el número al que llaman tus clientes. Al activar, te daremos un código para que las llamadas a este número las conteste tu recepcionista IA automáticamente.</p></div></div><Inp label="Email" value={d.email} onChange={v=>up('email',v)} placeholder="info@minegocio.com" type="email"/><Inp label="Dirección" value={d.address} onChange={v=>up('address',v)} placeholder="Calle Mayor 12, Madrid" hint="Opcional — útil para que el agente la diga"/><Inp label="Persona responsable" value={d.contact_person} onChange={v=>up('contact_person',v)} placeholder="Tu nombre" hint="¿A quién contactar si hay algo urgente?"/><div><label style={lbl}>Idioma principal</label><div style={{display:'flex',gap:8}}>{[{id:'es',l:'Español 🇪🇸'},{id:'ca',l:'Catalán'},{id:'eu',l:'Euskera'},{id:'en',l:'English 🇬🇧'}].map(lang=>(<button key={lang.id} onClick={()=>up('language',lang.id)} style={{padding:'8px 14px',borderRadius:8,fontSize:12,fontWeight:600,border:`1px solid ${d.language===lang.id?C.amber+'44':C.border}`,background:d.language===lang.id?C.amberDim:'transparent',color:d.language===lang.id?C.amber:C.sub,cursor:'pointer',fontFamily:'inherit'}}>{lang.l}</button>))}</div></div></div></div>)
  ...
  case 'activate':return(<div style={{textAlign:'center',maxWidth:500,margin:'0 auto'}}><div style={{fontSize:64,marginBottom:16}}>🚀</div><h1 style={{fontSize:28,fontWeight:800,color:C.text,lineHeight:1.3}}>¡Tu recepcionista <span style={{color:C.amber}}>{d.agent_name}</span> está lista!</h1><p style={{fontSize:15,color:C.sub,marginTop:16,lineHeight:1.6}}>Al activar, tu negocio empezará a recibir {L.booking.toLowerCase()}s automáticamente.</p><div style={{marginTop:32,display:'flex',flexDirection:'column',gap:12,textAlign:'left',background:C.card2,borderRadius:14,padding:'20px 24px'}}>{[`${d.agent_name} atenderá tus llamadas`,`Gestionará ${L.booking.toLowerCase()}s automáticamente`,d.reminders_enabled?`Enviará recordatorios a tus ${L.client.toLowerCase()}s`:null,d.channels.length>1?`Atenderá por ${d.channels.length} canales`:null,'Puedes cambiar todo después'].filter(Boolean).map((t,i)=>(<div key={i} style={{display:'flex',alignItems:'center',gap:10}}><span style={{color:C.green,fontSize:16}}>✓</span><span style={{fontSize:13,color:C.text}}>{t}</span></div>))}</div>{saving?(<div style={{marginTop:32,maxWidth:360,margin:'32px auto 0'}}><div style={{height:6,background:'rgba(255,255,255,0.06)',borderRadius:3,overflow:'hidden',marginBottom:12}}><div style={{height:'100%',width:`${((provisionStep+1)/provisionMessages.length)*100}%`,background:`linear-gradient(90deg,${C.amber},#E8923A)`,borderRadius:3,transition:'width 0.6s ease'}}/></div><p style={{fontSize:14,fontWeight:700,color:C.amber,minHeight:20,transition:'opacity 0.3s'}}>{provisionMessages[provisionStep]}</p></div>):(<button onClick={activate} style={{marginTop:32,padding:'16px 48px',fontSize:16,fontWeight:800,background:`linear-gradient(135deg,${C.amber},#E8923A)`,color:'#0C1018',border:'none',borderRadius:14,cursor:'pointer',fontFamily:'inherit',boxShadow:'0 4px 20px rgba(240,168,78,0.3)',transition:'all 0.2s'}}>Activar recepcionista</button>)}<p style={{fontSize:11,color:C.muted,marginTop:12}}>Puedes pausar o desactivar en cualquier momento</p></div>)
      ...
      <div style={{background:C.bg,minHeight:'100vh',fontFamily:"'Sora',-apple-system,sans-serif",display:'flex',alignItems:'center',justifyContent:'center'}}>
        <div style={{maxWidth:480,width:'100%',padding:32,textAlign:'center'}}>
          ...
          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:24,marginBottom:24,textAlign:'left'}}>
            ...
            <div style={{background:C.bg,borderRadius:12,padding:'20px 24px',display:'flex',alignItems:'center',justifyContent:'center',gap:12,marginBottom:16,border:`1px solid ${C.amber}33`}}>
              <span style={{fontSize:28,fontWeight:800,color:C.amber,letterSpacing:'0.08em',fontFamily:'monospace',flex:1,textAlign:'center'}}>{code}</span>
              <button onClick={()=>{navigator.clipboard.writeText(code);setCodeCopied(true);setTimeout(()=>setCodeCopied(false),2000)}} style={{padding:'8px 14px',borderRadius:8,border:`1px solid ${codeCopied?C.green+'44':C.amber+'44'}`,background:codeCopied?C.greenDim:C.amberDim,color:codeCopied?C.green:C.amber,fontSize:12,fontWeight:700,cursor:'pointer',fontFamily:'inherit',whiteSpace:'nowrap',transition:'all 0.2s'}}>{codeCopied?'✓ Copiado':'Copiar código'}</button>
            </div>
