    ...
    <div style={{ flex:1, display:'flex', flexDirection:'column', gap:16 }}>
      <div>
        <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:16 }}>
          ...
          <div>
            <p style={{ color:'#E8EEF6', fontWeight:600, fontSize:13 }}>Sofía · Recepcionista virtual</p>
            <p style={{ fontSize:11, color:'rgba(255,255,255,0.35)', marginTop:1 }}>Atendiendo llamadas ahora mismo</p>
          </div>
        </div>
        <h2 style={{ color:'#E8EEF6', fontSize:22, fontWeight:700, letterSpacing:'-0.03em', lineHeight:1.25, marginBottom:10 }}>Tu empleado que nunca duerme</h2>
        <p style={{ color:'rgba(255,255,255,0.38)', fontSize:13, lineHeight:1.7 }}>Responde, gestiona y registra cada llamada — y tú lo ves todo en el panel en tiempo real.</p>
      </div>
      <div style={{ flex:1, background:'rgba(0,0,0,0.3)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:14, overflow:'hidden', display:'flex', flexDirection:'column', minHeight:240 }}>
        <div style={{ padding:'9px 14px', borderBottom:'1px solid rgba(255,255,255,0.06)', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
          <div style={{ display:'flex', gap:5 }}>{['#ff5f57','#febc2e','#28c840'].map(c=><div key={c} style={{ width:8, height:8, borderRadius:'50%', background:c, opacity:0.6 }}/>)}</div>
            ...
            return (
              <div key={i} style={{ display:'flex', gap:7, justifyContent:isA?'flex-start':'flex-end' }}>
                ...
                <div style={{ maxWidth:'75%', background:isA?'rgba(255,255,255,0.06)':'rgba(240,168,78,0.15)', border:isA?'1px solid rgba(255,255,255,0.07)':'1px solid rgba(240,168,78,0.25)', borderRadius:isA?'4px 12px 12px 12px':'12px 4px 12px 12px', padding:'7px 11px' }}>
                  {typing===i?<div style={{ display:'flex', gap:3, alignItems:'center', height:14 }}>{[0,1,2].map(j=><div key={j} style={{ width:4, height:4, borderRadius:'50%', background:'rgba(255,255,255,0.4)', animation:'rz-bounce 1.1s ease-in-out infinite', animationDelay:j*0.18+'s' }}/>)}</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8 }}>
        {[{n:'24/7',l:'Sin descanso'},{n:'< 2s',l:'Tiempo respuesta'},{n:'100%',l:'Llamadas atendidas'}].map(s=>(
          <div key={s.l} style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:10, padding:'12px 8px', textAlign:'center' }}>
            <p style={{ fontFamily:"'JetBrains Mono',monospace", fontWeight:600, fontSize:16, color:'#E8EEF6' }}>{s.n}</p>
            <p style={{ color:'rgba(255,255,255,0.3)', fontSize:10.5, marginTop:3 }}>{s.l}</p>
          </div>
        ))}
      </div>
    </div>
...
export default function LoginPage() {
  ...
  async function doLogin() {
    ...
    try {
      const { data, error: err } = await supabase.auth.signInWithPassword({ email: emailVal, password: pwVal })
      if (err) throw err
