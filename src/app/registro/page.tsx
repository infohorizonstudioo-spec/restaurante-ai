...
function RegDemo() {
  ...
  useEffect(() => {
    ...
    const run = () => {
      ...
      CONV_REG.forEach((msg, i) => {
        const rt = msg.text.length * 26 + 350
        if (msg.from === 'agent') {
          timers.push(setTimeout(() => setTyping(i), t)); t += 850
          timers.push(setTimeout(() => { setTyping(null); setShown(p => [...p, i]) }, t)); t += rt
        } else {
          timers.push(setTimeout(() => setShown(p => [...p, i]), t)); t += rt
        }
      })
      timers.push(setTimeout(run, t + 2200))
    }
    run()
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%' }}>
      <div style={{ marginBottom:22 }}>
        ...
        <h2 style={{ color:'white', fontSize:20, fontWeight:700, letterSpacing:'-0.025em', lineHeight:1.3, marginBottom:7 }}>
      ...
      <div style={{ flex:1, background:'rgba(0,0,0,0.2)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:13, overflow:'hidden', display:'flex', flexDirection:'column', minHeight:0 }}>
        <div style={{ padding:'9px 13px', borderBottom:'1px solid rgba(255,255,255,0.05)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div style={{ display:'flex', gap:5 }}>
            {['#ff5f57','#febc2e','#28c840'].map(c => <div key={c} style={{ width:8, height:8, borderRadius:'50%', background:c, opacity:0.65 }}/>)}
          </div>
          <div style={{ display:'flex', alignItems:'center', gap:5 }}>
            <div style={{ width:5, height:5, borderRadius:'50%', background:'#f87171', animation:'pulse 1.5s infinite' }}/>
            <span style={{ fontSize:9.5, color:'rgba(255,255,255,0.28)', letterSpacing:'0.06em', textTransform:'uppercase', fontWeight:600 }}>En curso</span>
          </div>
        </div>
        <div ref={scrollRef} style={{ flex:1, padding:'14px', display:'flex', flexDirection:'column', gap:9, overflowY:'auto', scrollbarWidth:'none' }}>
          {CONV_REG.map((msg, i) => {
            const vis = shown.includes(i)
            const typ = typing === i
            if (!vis && !typ) return null
            if (msg.from === 'confirm' && vis) return (
              <div key={i} style={{ background:'rgba(74,222,128,0.09)', border:'1px solid rgba(74,222,128,0.18)', borderRadius:9, padding:'8px 12px', animation:'fadeUp 0.4s ease' }}>
                <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                  ...
                  <p style={{ fontSize:11.5, color:'#4ade80', fontWeight:600 }}>{msg.text}</p>
                </div>
              </div>
            )
            const isA = msg.from === 'agent'
            return (
              <div key={i} style={{ display:'flex', gap:7, justifyContent:isA?'flex-start':'flex-end', animation:'fadeUp 0.3s ease' }}>
                ...
                <div style={{ maxWidth:'78%', background:isA?'rgba(255,255,255,0.07)':'rgba(59,130,246,0.22)', border:isA?'1px solid rgba(255,255,255,0.06)':'1px solid rgba(59,130,246,0.28)', borderRadius:isA?'4px 11px 11px 11px':'11px 4px 11px 11px', padding:'7px 11px' }}>
                  {typ
                    ? <div style={{ display:'flex', gap:3, alignItems:'center', height:14 }}>{[0,1,2].map(j => <div key={j} style={{ width:4, height:4, borderRadius:'50%', background:'rgba(255,255,255,0.38)', animation:'bounce 1.1s ease-in-out infinite', animationDelay:j*0.18+'s' }}/>)}</div>
                    : <p style={{ fontSize:12, color:isA?'rgba(255,255,255,0.72)':'rgba(255,255,255,0.88)', lineHeight:1.55 }}>{msg.text}</p>
                  }
                </div>
              </div>
            )
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:9, marginTop:16 }}>
        {[
          { ico:'✓', t:'Responde igual que un recepcionista de confianza' },
          { ico:'✓', t:'Gestiona citas, reservas y consultas automáticamente' },
          { ico:'✓', t:'Tú ves todo en el panel en tiempo real' },
          { ico:'✓', t:'Sin contrataciones, sin horarios, sin imprevistos' },
        ].map(b => (
          <div key={b.t} style={{ display:'flex', alignItems:'flex-start', gap:9 }}>
            ...
            <p style={{ fontSize:12.5, color:'rgba(255,255,255,0.6)', lineHeight:1.5 }}>{b.t}</p>
          </div>
        ))}
      </div>
    </div>
    ...
    <div style={{ minHeight:'100vh', display:'grid', gridTemplateColumns:'1fr 1fr', fontFamily:"'DM Sans',-apple-system,sans-serif" }}>
      <style>{`
        ...
        *{box-sizing:border-box}
        ...
        .btn-m{width:100%;padding:12px;font-family:inherit;font-size:15px;font-weight:600;color:white;background:linear-gradient(135deg,#1e40af,#3b82f6);border:none;border-radius:9px;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;box-shadow:0 2px 10px rgba(59,130,246,0.28);transition:all 0.15s}
        .btn-m:hover:not(:disabled){opacity:0.93;transform:translateY(-1px)}
        .btn-m:disabled{opacity:0.55;cursor:default;transform:none}
        .tc{border:1px solid #e2e8f0;border-radius:9px;padding:10px 14px;cursor:pointer;transition:all 0.12s;background:#f9fafb;font-family:inherit;font-size:13px;text-align:left;color:#374151}
        .tc.sel{border-color:#3b82f6;background:#eff6ff;color:#1d4ed8;font-weight:600}
        .tc:hover:not(.sel){border-color:#94a3b8;background:#f1f5f9}
          ...
          {step===2 && (
            <div style={{ animation:'fadeUp 0.3s ease' }}>
                ...
                <div>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
                    {tipos.slice(0,6).map(([key,tmpl]) => (
                      <button key={key} onClick={()=>setForm({...form,businessType:key})} className={'tc'+(form.businessType===key?' sel':'')}>
                        {(tmpl as any).label}
                      </button>
                    ))}
                  </div>
              ...
              <div style={{ display:'flex', gap:10 }}>
                <button onClick={()=>setStep(1)} style={{ flex:1, padding:'12px', fontFamily:'inherit', fontSize:14, fontWeight:500, color:'#374151', background:'white', border:'1px solid #e2e8f0', borderRadius:9, cursor:'pointer' }}>
                  ← Atrás
                </button>
                <button className="btn-m" style={{ flex:2 }} disabled={loading} onClick={handleRegister}>
      ...
      <div className="rp" style={{ background:'linear-gradient(160deg,#0b1120 0%,#0f1f3d 45%,#0b1120 100%)', minHeight:'100vh', display:'flex', flexDirection:'column', justifyContent:'center', padding:'clamp(32px,5vw,60px)', position:'relative', overflow:'hidden' }}>
        <div style={{ position:'absolute', inset:0, opacity:0.022, backgroundImage:'linear-gradient(rgba(255,255,255,0.6) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,0.6) 1px,transparent 1px)', backgroundSize:'40px 40px', pointerEvents:'none' }}/>
        <div style={{ position:'absolute', top:'8%', right:'8%', width:260, height:260, background:'radial-gradient(circle,rgba(59,130,246,0.07),transparent)', borderRadius:'50%', pointerEvents:'none' }}/>
        <div style={{ position:'relative', zIndex:1, height:'min(580px,82vh)', display:'flex', flexDirection:'column' }}>
          <RegDemo/>
        </div>
      </div>
    </div>
