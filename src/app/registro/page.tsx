      ...
      <div className="relative w-full max-w-md">
        <div className="text-center mb-8">
          <Link href="/login" className="inline-flex items-center gap-2 mb-2">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center text-xl font-black shadow-2xl shadow-violet-500/30">R</div>
          </Link>
        ...
        <form onSubmit={handleSubmit}>
          <div className="bg-white/[0.04] border border-white/[0.08] rounded-2xl p-6 space-y-4 backdrop-blur-sm">
            ...
            <div>
              <label className="text-xs text-white/40 mb-1.5 block">Nombre del negocio *</label>
              <input type="text" value={form.businessName} onChange={e => update('businessName', e.target.value)} placeholder="Ej: Restaurante La Bahía" autoFocus
            ...
            <div>
              <label className="text-xs text-white/40 mb-2 block">Tipo de negocio *</label>
                ...
                {TIPOS.map(t => (
                  <button key={t.value} type="button" onClick={() => update('businessType', t.value)}
                    className={`flex items-center gap-2 px-3 py-2.5 rounded-xl border text-sm transition-all text-left
                      ${form.businessType === t.value ? 'bg-violet-600/20 border-violet-500/50 text-white' : 'bg-white/5 border-white/10 text-white/50 hover:border-white/25 hover:text-white/70'}`}>
            ...
            <div>
              ...
              <input type="email" value={form.email} onChange={e => update('email', e.target.value)} placeholder="tu@negocio.com"
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-white/20 focus:outline-none focus:border-violet-500/60 transition-all"/>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-white/40 mb-1.5 block">Contraseña *</label>
                <input type="password" value={form.password} onChange={e => update('password', e.target.value)} placeholder="Mín. 8 caracteres"
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-white/20 focus:outline-none focus:border-violet-500/60 transition-all"/>
              </div>
              <div>
                <label className="text-xs text-white/40 mb-1.5 block">Confirmar *</label>
                <input type="password" value={form.confirmPassword} onChange={e => update('confirmPassword', e.target.value)} placeholder="Repetir"
                  className={`w-full bg-white/5 border rounded-xl px-4 py-3 text-sm text-white placeholder-white/20 focus:outline-none transition-all ${form.confirmPassword && form.password !== form.confirmPassword ? 'border-red-500/50' : 'border-white/10 focus:border-violet-500/60'}`}/>
              </div>
            </div>
            <div>
              <label className="text-xs text-white/40 mb-1.5 block">Teléfono <span className="text-white/25">(opcional)</span></label>
              <input type="tel" value={form.phone} onChange={e => update('phone', e.target.value)} placeholder="+34 600 000 000"
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-white/20 focus:outline-none focus:border-violet-500/60 transition-all"/>
            </div>
            <button type="submit" disabled={loading}
              className="w-full bg-violet-600 hover:bg-violet-500 disabled:opacity-50 text-white font-bold py-3.5 rounded-xl text-sm transition-all active:scale-95">
              {loading ? <span className="flex items-center justify-center gap-2"><svg className="animate-spin w-4 h-4" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>Creando tu cuenta...</span> : 'Crear cuenta gratis →'}
            </button>
          </div>
        </form>
        <div className="mt-5 text-center space-y-2">
          ...
          <p className="text-xs text-white/20"><Link href="/precios" className="hover:text-white/40">Ver planes y precios</Link></p>
        </div>
      </div>
    </div>
  )
