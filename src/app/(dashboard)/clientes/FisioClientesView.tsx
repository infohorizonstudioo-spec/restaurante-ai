...
export default function FisioClientesView() {
  ...
  const load = useCallback(async (tenantId:string) => {
    ...
    const {data:reservations} = await supabase.from('reservations').select('customer_id,date,status')
      ...
      <div style={{display:'flex',flex:1,overflow:'hidden'}}>
        ...
        <div style={{width:320,flexShrink:0,overflowY:'auto',borderRight:`1px solid ${C.border}`,background:C.surface}}>
          {filtered.length===0 ? (
            ...
            const active = futureCitas[c.id] || false
            return (
              <div key={c.id} onClick={()=>openClient(c)} style={{padding:'12px 16px',cursor:'pointer',borderBottom:`1px solid ${C.border}`,
                ...
                <div style={{display:'flex',alignItems:'center',gap:10}}>
                  <div style={{width:36,height:36,borderRadius:'50%',background:C.tealDim,display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,fontWeight:700,color:C.teal,flexShrink:0}}>
            ...
            <>
              <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:20,marginBottom:16}}>
                <div style={{display:'flex',alignItems:'center',gap:14,marginBottom:16}}>
                  <div style={{width:50,height:50,borderRadius:'50%',background:C.tealDim,display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,fontWeight:700,color:C.teal}}>
                ...
                <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12}}>
                  {[
                    ...
                    <div key={m.label} style={{background:C.surface2,borderRadius:9,padding:'10px 14px'}}>
                      ...
                      <p style={{fontFamily:'var(--rz-mono)',fontSize:m.label==='Estado'?13:18,fontWeight:700,color:m.label==='Estado'?(futureCitas[selected.id]?C.green:C.text3):C.text}}>{m.value}</p>
                    </div>
                  ))}
                </div>
