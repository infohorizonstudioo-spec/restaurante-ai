...
export default function AcademiaAlumnosView() {
          ...
          {filtered.length===0 ? (
            ...
            <div key={c.id} onClick={()=>openClient(c)} style={{padding:'12px 16px',cursor:'pointer',borderBottom:`1px solid ${C.border}`,
              ...
              <div style={{display:'flex',alignItems:'center',gap:10}}>
                <div style={{width:36,height:36,borderRadius:'50%',background:c.vip?C.yellowDim:C.blueDim,display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,fontWeight:700,color:c.vip?C.yellow:C.blue,flexShrink:0}}>
            ...
            <>
              ...
              <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:20,marginBottom:16}}>
                <div style={{display:'flex',alignItems:'center',gap:14,marginBottom:16}}>
                  <div style={{width:50,height:50,borderRadius:'50%',background:selected.vip?C.yellowDim:C.blueDim,display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,fontWeight:700,color:selected.vip?C.yellow:C.blue}}>
              ...
              : historial.map((h,i)=>(
                <div key={i} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:'12px 14px',marginBottom:8,display:'flex',gap:10,transition:'background 0.12s'}}
                  ...
                  <span style={{fontSize:16}}>{h._type==='clase'?'📚':'📞'}</span>
                  <div style={{flex:1}}>
                    {h._type==='clase' ? (
                      <>
                        ...
                        <p style={{fontSize:11,color:C.text3,marginTop:1}}>
                          {h.table_name?'Aula: '+h.table_name+' · ':''}{h.status}
                        </p>
                      </>
                    ) : (
                      <>
