...
const LEVEL_STYLES:Record<string,{bg:string;color:string}> = {
  ...
  intermedio:   {bg:C.blueDim,  color:C.blue},
  avanzado:     {bg:C.violetDim,color:C.violet},
}
...
function extractLevel(notes?: string): string | null {
  if (!notes) return null
  const lower = notes.toLowerCase()
  if (lower.includes('avanzado')) return 'avanzado'
  if (lower.includes('intermedio')) return 'intermedio'
  if (lower.includes('principiante') || lower.includes('básico') || lower.includes('basico')) return 'principiante'
  return null
}

function extractSubject(r: any): string {
  return r.service || r.subject || ''
}

export default function AcademiaClasesView() {
  ...
  const filtered = search ? dayRes.filter(r =>
    ...
    (extractSubject(r)||'').toLowerCase().includes(search.toLowerCase())
  ) : dayRes
            ...
            <div key={r.id} onClick={()=>setModal(r)} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:'14px 16px',marginBottom:10,cursor:'pointer',display:'flex',alignItems:'center',gap:12,transition:'all 0.12s'}}
              ...
              <div style={{flex:1,minWidth:0}}>
                <div style={{display:'flex',alignItems:'center',gap:6}}>
                  <p style={{fontSize:14,fontWeight:600,color:C.text}}>{studentName}</p>
                  {subject&&<span style={{fontSize:10,padding:'2px 7px',borderRadius:6,background:C.blueDim,color:C.blue,fontWeight:600}}>📖 {subject}</span>}
                </div>
